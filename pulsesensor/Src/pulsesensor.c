...
void pulseSensorCycleDepartureCallback()
{
	// 原代码中是放入定时器回调函数中 
...
void pulseSensorSendDataToProcessing(char symbol, int dat )
{
    putchar(symbol);                // symbol prefix tells Processing what type of data is coming
		printf("%d\r\n",dat);						// the data to send culminating in a carriage return
}

void pulseSensorMain(void)
{
	// 将上述回调在定时器中断中使用后
	// 这个函数直接在主循环中调用
	pulseSensorSendDataToProcessing('S', Signal);     // send Processing the raw Pulse Sensor data
	if (QS == true)
	{
			pulseSensorSendDataToProcessing('B',BPM);   // send heart rate with a 'B' prefix
			pulseSensorSendDataToProcessing('Q',IBI);   // send time between beats with a 'Q' prefix
